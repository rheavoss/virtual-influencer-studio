...
- [x] **[P0-37]** `[Tech Ops]` (Claude Code) *(RICE: 8×8×1.0÷1 = 64)* — ✅ DONE 2026-04-20 — Fixed deploy command in `CLAUDE.md`: changed `name:"jasmine-dashboard"` → `name:"virtual-influencer-dashboard"`. (`deploy.md` was already correct.)
- [ ] **[P0-38]** `[Tech Ops]` (Claude Code) *(RICE: 9×9×1.0÷2 = 40.5)* — **URGENT** Delete legacy `jasmine-dashboard` Vercel project (prj_3cCte5hEyZuJpwzDQ7EGocr4Gxc1). Character-specific name conflicts with multi-character agency. **Pre-condition:** Must first deploy to `virtual-influencer-dashboard` project and re-alias production URL (alias currently points to jasmine-dashboard deployment). Do after P0-36.
...
- [x] **[P0-39]** `[AI Ops]` (Claude) *(RICE: 10×10×1.0÷1 = 100)* — ✅ DONE 2026-04-26 (Session 22) — LAMA inpainting on Colab T4. Watermark-only mask (40% width × 10% height). All 62 images cleaned. Output in `02_training_data/v4_clean_lama/`.
- [x] **[P0-40]** `[AI Ops]` (Grok) *(RICE: 10×10×1.0÷1 = 100)* — ✅ RESOLVED 2026-04-25 — Grok provided v4 master caption block (Session 20). Saved to `03_ai_models/jasmine_mako/02_training_data/v4_clean/MASTER_CAPTION_V4.md`. Applied to all 62 .txt caption files. Caption: `jasmakogirl, 24 year old stunning East Asian Japanese-Korean beauty, porcelain fair flawless skin with zero imperfections whatsoever, uniform pale skin tone, no spots no blemishes no tan lines no pores, glossy wet-look skin sheen, heavy natural DD breasts with realistic gravity and soft tissue weight, natural teardrop shape, subtle natural sag, lower breast attachment point, realistic underboob shadow and softness, visible breast weight and movement under gravity, slim waist, flat toned stomach, lean petite hourglass figure 52kg, perfect symmetrical face, heavy seductive makeup, glossy red lips, long glossy dark hair in high messy bun with loose strands, photorealistic, ultra detailed, 8k`
- [x] **[P0-41]** `[Strategy]` (CEO + Grok) *(RICE: 10×10×1.0÷1 = 100)* — ✅ RESOLVED 2026-04-25 — Decision: train v4 on all 62 images. 62 PNGs + 62 caption .txt files staged in `03_ai_models/jasmine_mako/02_training_data/v4_clean/` (local only, gitignored by design). Next blocker: P0-39 LAMA cleanup (awaiting CEO approval).
...
### Post-LoRA Content Launch (Sequential: After P0-44)
- [ ] **[P0-45]** `[Content]` (Claude + CEO) *(RICE: 9×9×0.9÷2 = 36.5)* — **BLOCKED ON P0-44** Generate 1 sample image for each of CT-1 through CT-11 using `JASMINE_PROMPT_LIBRARY.md`. Run every prompt against jasmine_v4.safetensors. Output to `04_content_pipeline/jasmine/ct_samples/`. This is the LoRA acceptance test — if any CT fails, flag before moving to production.
- [ ] **[P0-46]** `[Tech Ops]` (Antigravity + Claude) *(RICE: 8×8×1.0÷1 = 64)* — Lock 4K export settings for all platforms (IG Feed, IG Stories, Reels, Fanvue). Document exact resolution, bitrate, codec, and aspect ratio per platform. Output to `04_content_pipeline/jasmine/export_settings.md`. Reference: `JASMINE_CONTROLNET_AND_UPSCALE_QUICKSTART.md` (2-step upscale spec already defined).
- [ ] **[P0-47]** `[Content]` (Claude + CEO) *(RICE: 9×10×0.9÷3 = 27)* — **BLOCKED ON P0-45** Run full end-to-end test of one complete content day: generate → OPSEC strip → caption → schedule via Metricool. Confirm every step works before the 7-day batch. Log any failures before launch.
- [ ] **[P0-48]** `[Exec Dept]` (CEO) *(RICE: 10×10×1.0÷4 = 25)* — **BLOCKED ON P0-47** Launch first 7-day content batch on Instagram. Execute per `JASMINE_CONTENT_CALENDAR_AND_7_PROMPT_SYSTEM.md` Day 1–7. Go-live milestone for Phase 0.
