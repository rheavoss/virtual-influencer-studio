# SONIA_FLUX_LORA_TRAINING_GUIDE_v2_EXHAUSTIVE.md

**Date**: 22 April 2026  
**Version**: 2.0 (Exhaustive Edition – Post-Failure Analysis)  
**Project**: Sonia – Girl-next-door GFE Virtual Influencer  
...
You are assisting the Sonia GFE project.  
...
- [ ] Skin is flawless on every image
...
- Flawless smooth skin, zero blemishes, zero shine, zero texture, zero pores, zero discoloration
- Exact Sonia face (consistent features)
...
**Trigger Word**: `soniagfe` (unique, easy to remember, same as LoRA name)

**Caption Strategy** (choose one):

- Option A (fast): Just `soniagfe` on every image
...
4. Base prompt template:
   soniagfe, [full base prompt above], wearing black micro bikini, standing in bedroom, seductive pose, looking at viewer
...
This is now the single source of truth for all Sonia LoRA training.
