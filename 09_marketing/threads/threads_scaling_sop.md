# Threads Scaling SOP — Sonia Subscriber Acquisition
**Source:** Master Frameworks Compilation — Framework 4
**Task:** P1-50
**Owner:** CEO / Antigravity
**Phase:** Phase 1 (after IG account live)

---

## Goal

Generate 20–100+ paying Fanvue subscribers per day per strong Threads account using zero-ad organic strategy.

---

## 7-Day Account Warm-Up Process

| Day | Action |
|---|---|
| Day 1 | Create fresh IG + Threads accounts with US/UK SIM. Follow 50 accounts in GFE / beauty / fitness niche. NO posts yet. |
| Day 2 | Comment on 8–10 viral posts in niche. Comments must be genuine, high-quality (not "great post!"). |
| Day 3 | Post 1 amateur-style AI photo of Jasmine (no watermarks, no obvious AI tells). Caption: relatable/personal. |
| Day 4 | Post 2–3 times. Comment on 8 viral posts. Reply to anyone who engaged Day 3. |
| Day 5 | Post 3 times. Add link.boo URL to bio. DM 5 people who liked your posts. |
| Day 6 | Post 3–4 times. Comment 8+. First soft CTA in caption: "link in bio for more." |
| Day 7 | Full speed: 5–6 posts/day. Hard CTA in 1 post. Comment 10+. |

---

## Content Formula

**Post types (rotate):**
1. **Amateur selfie-style** — casual pose, personal caption ("just woke up", "lazy Sunday")
2. **Behind-the-scenes** — "how I edit my photos" or "my morning routine"
3. **Engagement bait** — question in caption ("beach or pool?")
4. **Soft tease** — "exclusive content on my Fanvue 👀"
5. **Social proof** — "can't believe the response on my last post 🙈"

**Caption rules:**
- 2–4 sentences max
- First-person voice (Jasmine's POV)
- 1–2 emojis max
- No hashtags on Threads (they reduce reach)
- Always reply to every comment within 1 hour for first 24h

---

## Link-in-Bio Setup

```
link.boo/jasminemako
  ├── Fanvue (primary CTA)
  ├── Free preview gallery
  └── Instagram (cross-promote)
```

---

## Scaling: Multiple Accounts

Once one account proven:
- Launch 2nd account with different aesthetic (e.g. "gym Jasmine" vs "beach Jasmine")
- Same character, different persona angle
- Each account funnels to same Fanvue

**Max accounts recommended:** Start with 1. Scale to 3 once first is profitable.

---

## Simp Hunting (No DMs Until Day 5+)

- Identify high-engagement followers (likes + comments)
- Tag them in relevant posts ("this reminded me of you @username")
- Move to DMs only after 3+ interactions

---

## KPIs to Track

| Metric | Target by Day 30 |
|---|---|
| Followers | 1,000+ |
| Engagement rate | 5%+ |
| Link clicks/day | 20+ |
| Fanvue conversions | 5–10 subscribers |
