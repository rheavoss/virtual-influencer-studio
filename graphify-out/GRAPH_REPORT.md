# Graph Report - /Users/user/Desktop/Instagram  (2026-04-24)

## Corpus Check
- Corpus is ~22,596 words - fits in a single context window. You may not need a graph.

## Summary
- 94 nodes · 105 edges · 15 communities detected
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 14 edges (avg confidence: 0.81)
...
2. `Jasmine Project (CLAUDE.md)` - 9 edges
3. `Hybrid Workflow (Grok + Claude + Antigravity)` - 8 edges
4. `Programmatic SEO Strategy (P1-48)` - 7 edges
5. `Jasmine Character (AI Influencer)` - 6 edges
6. `RALPH v16.1-jasmine (Governance System)` - 6 edges
7. `GeoDirectory SEO Flywheel (P1-49)` - 6 edges
8. `getData()` - 5 edges
9. `Fanvue Platform (GFE Revenue)` - 5 edges
10. `Deploy Guide — Jasmine P&L Dashboard` - 5 edges

## Surprising Connections (you probably didn't know these)
- `Jasmine Character Spec (jasmakogirl)` --defines--> `Jasmine Character (AI Influencer)`  [INFERRED]
  99_grok_brain/GROK_BRAIN_SUMMARY.md → CLAUDE.md
- `RALPH v16.1-jasmine (Governance System)` --gatekeeper_is--> `Grok (Research Gatekeeper)`  [INFERRED]
  99_grok_brain/GROK_VERIFY_RALPH.md → CLAUDE.md
- `GROK BRAIN — Jasmine Project State` --single_source_of_truth_for--> `Jasmine Project (CLAUDE.md)`  [EXTRACTED]
  99_grok_brain/GROK_BRAIN_SUMMARY.md → CLAUDE.md
- `Hybrid Workflow (Grok + Claude + Antigravity)` --coordinates--> `Antigravity (Research/Copy/Captions)`  [EXTRACTED]
  90_protocols/HYBRID_WORKFLOW.md → CLAUDE.md
- `Hybrid Workflow (Grok + Claude + Antigravity)` --coordinates--> `Claude Code (Files/Code/Memory)`  [EXTRACTED]
  90_protocols/HYBRID_WORKFLOW.md → CLAUDE.md

## Hyperedges (group relationships)
- **Jasmine Revenue Funnel** — claude_md_instagram_platform, claude_md_fanvue_platform, programmatic_seo, geo_directory_seo, threads_scaling_sop, ai_personas_strategy, youtube_rpv_strategy [INFERRED 0.85]
- **RALPH Governance System** — grok_verify_ralph_system, grok_verify_ralph_gate_enforcer, grok_verify_ralph_auto_merge, grok_verify_ralph_brain_sync, ralph_checklist, grok_brain_g0_checklist [EXTRACTED 1.00]
- **SEO Strategy Cluster** — seo_readme, seo_three_layer, programmatic_seo, geo_directory_seo [EXTRACTED 1.00]
- **LoRA Training Pipeline** — grok_brain_dataset_v2, grok_brain_kling_tool, grok_brain_flux_dev, grok_brain_lora_v2_pending, grok_verify_ralph_vast_ai [EXTRACTED 1.00]
- **Team Handoff Channels** — grok_outbox, claude_inbox, antigravity_inbox, grok_inbox, handoff_template [EXTRACTED 1.00]
